  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-6 py-8">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-8 shadow-sm">
        <h1 className="mb-6 text-center text-2xl font-bold text-foreground">{t("login.title")}</h1>
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required
                className="w-full rounded-lg border border-border bg-background px-3 py-2 pr-10 text-sm text-foreground outline-none focus:border-foreground/40 focus:ring-2 focus:ring-brand-lime-bright/20" />
        ...
        <p className="mt-4 text-center text-sm text-muted-foreground">
          {t("login.no_account")}{" "}
          <Link href={`/${lng}/register`} className="font-medium text-foreground hover:underline">
